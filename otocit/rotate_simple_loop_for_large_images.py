...
for nazev_snimku in os.listdir(INPUT_DIR):
    ...
    if not (nazev_snimku.lower().endswith(".jpg") or
            nazev_snimku.lower().endswith(".jpeg") or
            nazev_snimku.lower().endswith(".png") or
            nazev_snimku.lower().endswith(".tif") or
            nazev_snimku.lower().endswith(".tiff")):
        continue
    # end if

    # show the file name on std out
    print("zpracovava se soubor " + nazev_snimku)

    # get the file name and full path of the current image file
    imageFileWithPath = os.path.join(INPUT_DIR, nazev_snimku)
    ...
    for uhel_otoceni in uhly:
        ...
        obrazek_hotovy = cv2.addWeighted(snimek_otoceny_a_orezany_a_doostreny, 1 - mira_pruhlednosti, snimek_otoceny_a_orezany,
                                         mira_pruhlednosti, 0)
